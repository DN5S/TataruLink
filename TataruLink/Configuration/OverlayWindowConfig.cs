...
public class OverlayWindowConfig
{
    ...
    /// <summary>
    /// Ensures all default chat type colors are present
    /// </summary>
    public void EnsureDefaultColors()
    {
        var defaults = GetDefaultChatTypeColors();
        foreach (var kvp in defaults)
        {
            ChatTypeColors.TryAdd(kvp.Key, kvp.Value);
        }
    }
    
    private static Dictionary<ushort, Vector4> GetDefaultChatTypeColors()
    {
        return new Dictionary<ushort, Vector4>
        {
            [(ushort)XivChatType.Say] = Colors.White,
            [(ushort)XivChatType.Shout] = Colors.Orange,
            [(ushort)XivChatType.TellIncoming] = Colors.Pink,
